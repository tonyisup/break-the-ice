...
def verify_feature(page):
    ...
    try:
        count_text = page.locator("p:has-text('Liked Count:')").text_content()
        print(f"Initial: {count_text}")
    except:
        print("Could not find count text")
    ...
    try:
        count_text = page.locator("p:has-text('Liked Count:')").text_content()
        print(f"After Fill: {count_text}")
    except:
        print("Could not find count text")

    # Wait for badge to appear (Red badge for limit reached)
    print("Checking for limit badge...")
    # The badge is in Header. TestVerificationPage includes Header.
    # Selector: .bg-red-500.rounded-full.border-2

    # Take screenshot to see what's happening
    page.screenshot(path="verification/debug_badge.png")
    ...
    # Ensure filled again (localStorage persists, but good to be safe)
    page.wait_for_selector("text=Fill Likes to Limit")
    page.click("text=Fill Likes to Limit")
