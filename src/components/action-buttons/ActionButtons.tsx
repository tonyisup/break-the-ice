    ...
    <>
      {isStyleTonesOpen ? (
          ...
          <div className="flex gap-4">
            ...
            >
              <button
                ...
                className={cn(
                  "text-black dark:text-white bg-white dark:bg-black",
                  " px-5 py-3 rounded-full flex items-center gap-2 hover:bg-white/80 dark:hover:bg-black/80 transition-colors"
                )}
            ...
            {isHighlighting &&
              <div
                style={{
                  background: `linear-gradient(135deg, ${shuffledGradient[1]}, ${shuffledGradient[0]})`,
                }}
                className="px-1 py-1 rounded-full"
              >
                <button
        ...
        <div className="flex justify-center p-4 gap-4">
          <div
            style={{
              background: `conic-gradient(from 270deg, ${allColors.sort(() => isShuffling ? Math.random() - 0.5 : 0).map((color, index) =>
                `${color} ${(index / (allColors.length - 1)) * 100}%`
              ).join(', ')})`,
            }}
            className="px-1 py-1 rounded-full"
