...
export default function QuestionsPage() {
  ...
  const [remixingIds, setRemixingIds] = React.useState<Set<Id<"questions">>>(
    new Set(),
  );
  ...
  const [editingQuestion, setEditingQuestion] =
    React.useState<Doc<"questions"> | null>(null);
  ...
  const filteredQuestions = activeQuestions.filter((q) => {
    ...
    const toneName =
      tones?.find((t) => t.id === q.tone)?.name.toLowerCase() || "";
    const searchLower = search.toLowerCase();
    return (
      text.includes(searchLower) ||
      styleName.includes(searchLower) ||
      toneName.includes(searchLower)
    );
  });
  ...
  const rangeStart =
    filteredQuestions.length === 0 ? 0 : (page - 1) * pageSize + 1;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage and moderate questions in the database.
          </p>
        </div>
          ...
          >
            ...
            <DialogContent className="sm:max-w-[500px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  Add a new manually curated question to the pool.
                </DialogDescription>
              </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewQuestion({ ...newQuestion, tone: e.target.value })
                      }
                      className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                    >
                      {tones.map((t) => (
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4">
            {pendingQuestions.map((q) => (
              <div
                key={q._id}
                className="p-4 md:p-6 bg-white dark:bg-gray-800 rounded-xl border-2 border-primary/20 shadow-sm flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 transition-all hover:border-primary/40"
              >
              ...
              <tbody className="divide-y block md:table-row-group">
                {visibleQuestions.map((q) => {
                  const style = styles.find(
                    (s) => s._id === q.styleId || s.id === q.style,
                  );
                  const tone = tones.find(
                    (t) => t._id === q.toneId || t.id === q.tone,
                  );
                  ...
                  return (
                    <tr
                      key={q._id}
                      className="block md:table-row hover:bg-muted/30 transition-colors group"
                    >
                      <td className="block md:table-cell px-4 md:px-6 py-3 md:py-4">
                        ...
                        >
                          <code className="text-xs text-primary/80 hover:text-primary transition-colors">
                            {q._id}
                          </code>
                        </Link>
                      </td>
                      ...
                      <td className="block md:table-cell px-4 md:px-6 py-2 md:py-4">
                        ...
                        <Badge
                          variant="secondary"
                          className="capitalize text-[10px] font-normal px-2 py-0"
                        >
                          {q.status || "public"}
                        </Badge>
                      </td>
                        ...
                        <div className="flex items-center justify-start md:justify-end gap-2 transition-opacity">
                          {isEditing ? (
                            <div className="flex gap-2 w-full md:w-auto">
                              ...
                              <Button
                                variant="outline"
                                className="flex-1 md:flex-none gap-2 text-red-600 border-red-200 bg-red-50"
                              ...
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <Button
                                    variant="ghost"
                                    size="icon"
                                    className="size-9 border md:border-0"
                                  >
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="-mr-1 !size-11 shrink-0 opacity-100 transition-opacity md:!size-8 md:opacity-0 md:group-hover:opacity-100"
                          >
                  ...
                  {!isEditing && (
                    <div className="mt-4 pt-4 border-t border-muted flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      <span className="flex items-center">
                        ...
                        <Link
                          to={`/admin/questions/${q._id}`}
                          className="inline-flex min-h-11 items-center pl-1 text-primary/80 hover:text-primary hover:underline md:min-h-0"
                        >
                          {q._id.slice(0, 8)}...
                        </Link>
                      </span>
