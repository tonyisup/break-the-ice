...
export const getStyle = query({
	...
	handler: async (ctx, args) => {
		...
		const { organizationId: _o, embedding: _e, ...rest } = style;
		return rest;
	},
});
...
export const getStyles = query({
	...
	handler: async (ctx, args) => {
		const styles = await ctx.db
			...
			.collect();
		return styles.map(({ organizationId: _o, embedding: _e, ...rest }) => rest);
	},
});
...
export const getRandomStyle = query({
	...
	handler: async (ctx, args) => {
		...
		const { organizationId: _o, embedding: _e, ...rest } = randomStyle;
		return rest;
	},
});
