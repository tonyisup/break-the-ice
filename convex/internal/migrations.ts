...
export const removeOldTimestampFields = internalMutation({
	...
	handler: async (ctx) => {
		...
		for (const f of feedbacks) {
			if ("createdAt" in f) {
				const { createdAt: _createdAt, _id, _creationTime, ...rest } = f as any;
		...
		for (const s of subscriptions) {
			if ("createdAt" in s) {
				const { createdAt: _createdAt, _id, _creationTime, ...rest } = s as any;
		...
		for (const p of progress) {
			if ("startedAt" in p) {
				const { startedAt: _startedAt, _id, _creationTime, ...rest } = p as any;
		...
		for (const d of detections) {
			if ("detectedAt" in d) {
				const { detectedAt: _detectedAt, _id, _creationTime, ...rest } = d as any;
