...
describe("Daily newsletter", () => {
  test("returns unique question on each call", async () => {
    ...
    await t.mutation(internal.questions.assignPoolQuestionsToUser, {
        userId,
    ...
    const result1 = await t.query(api.questions.getQuestionForNewsletter, {
        userId
    });
    ...
    const result2 = await t.query(api.questions.getQuestionForNewsletter, {
        userId
    });
    ...
    const result3 = await t.query(api.questions.getQuestionForNewsletter, {
        userId
    });
